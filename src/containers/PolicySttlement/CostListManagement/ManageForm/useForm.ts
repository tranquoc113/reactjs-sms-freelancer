...
import costListActions from 'src/store/domain/costList';
...
const useForm = () => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    to_date: Yup.date()
      ...
      .required(),
    active_agency: Yup.string().nullable(true).required(),
  ...
  const formik = useFormik<any>({
    ...
    validationSchema,
    onSubmit(value) {
      dispatch(costListActions.setCostList(true));
    },
  ...
  const handleClear = () => {
    ...
    formik.submitForm();
    dispatch(costListActions.setCostList(false));
  };
