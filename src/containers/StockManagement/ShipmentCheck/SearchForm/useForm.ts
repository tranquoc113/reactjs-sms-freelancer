...
const useForm = () => {
  ...
  const formik = useFormik<SearchParams.ShipmentCheck>({
    initialValues: {
      ...defaultValue,
      ...query,
    },
  ...
  const onChangeStateCls = (checkedValue: Array<CheckboxValueType>) => {
    formik.setFieldValue('state_cls', checkedValue);
  };

  return { formik, onChangeQuickPeriod, handleClear, onChangeStateCls };
};
