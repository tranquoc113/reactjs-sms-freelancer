 ...
 * 	{myLessVariable:123}
 */
export default function getLessVars(id, parseNumbers) {
  var bNumbers = parseNumbers === undefined ? true : parseNumbers,
    ...
    rgUnit = /[a-z]+$/,
    aUnits =
      'em,ex,ch,rem,vw,vh,vmin,cm,mm,in,pt,pc,px,deg,grad,rad,turn,s,ms,Hz,kHz,dpi,dpcm,dppx'.split(
        ',',
      ),
    rgValue = /:\s?(.*)\s?;\s?\}/,
    rgStr = /^'([^']+)'$/,
